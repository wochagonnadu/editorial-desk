...
Легенда зависимостей: `dep:011` — история зависит от спеки `011-expert-onboarding-5-step-hardening`.
...
4. [partial][should] Как пользователь, я хочу разлогиниться в 1 клик из приложения, чтобы безопасно завершить сессию на общем устройстве.
...
5. [partial][must] Как гость, я хочу нормальный мобильный скролл без обрезаний контента, чтобы лендинг был полностью доступен.
...
2. [partial][must] Как пользователь, я хочу переходить из Home в Drafts/Experts/Calendar/Approvals/Settings без тупиков, чтобы управлять ежедневным потоком.
...
4. [partial][must][dep:011] Как пользователь, я хочу чтобы email-onboarding эксперта проходил шаги 1->5 без обрыва, чтобы voice acquisition был рабочим.
5. [partial][should][dep:011] Как пользователь, я хочу видеть статус готовности voice profile после прохождения шагов 1->5, чтобы понимать, когда можно генерировать в голосе эксперта.
...
1. [partial][must] Как пользователь, я хочу адаптивный интерфейс без перекрытий на мобильном, чтобы выполнять базовые сценарии с телефона.
2. [partial][should] Как пользователь, я хочу плавные анимации без лагов, чтобы интерфейс не мешал чтению и правке.
3. [partial][must] Как пользователь, я хочу явные состояния загрузки и ошибок в ключевых действиях, чтобы понимать что делать дальше.
4. [partial][must] Как пользователь, я хочу предсказуемую навигацию и стабильный возврат назад, чтобы не терять контекст.
...
- Истории с тегом `dep:011` отдельно вынесены в план `011-expert-onboarding-5-step-hardening`, потому что в текущих ручных проверках подтвержден только Step 1.
