...
| TD-013 | Topics + Factcheck UX | В web не было явного UI для `topics.suggest`, а в редакторе исторически смешивались flow фактчека и отправки на approval; контекст проверяемых claim/evidence был неполным. | Пользователь не понимал, что именно проверено и когда можно отправлять на review; часть LLM-возможностей была недоступна из UI. | Закрыто в Spec 022: добавлен `Suggest topics` flow, раздельные шаги `Run factcheck`/`Send for approval`, claims/evidence panel и live smoke на Vercel. | High      | closed |
| TD-014 | Expert entry surface | Для эксперта пока не зафиксировано жесткое правило “без основного UI”; это может снова увести продукт в лишний кабинет для review/onboarding. | Эксперту придется осваивать интерфейс, хотя по продуктовой модели он должен работать email-first и через ссылку на документ. | Зафиксировать отдельную фичу `open document by email link only`: эксперт работает через письмо и public-doc ссылку из письма, без захода в основной app shell. | High      | open   |
