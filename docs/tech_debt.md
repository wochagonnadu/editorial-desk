...
| TD-011 | Auth runtime (prod) | Для обхода Vercel timeout login временно принимает `email` из query (`?email=`).         | Email попадает в URL-логи (PII), контракт login временно неканоничный.          | Закрыто 2026-03-03: web login возвращен на JSON body-only, `POST /api/v1/debug/json-echo` удален, RCA обновлен. Ссылка: `specs/007-vercel-auth-json-body-recovery/tasks.md` (Phase A-E). | High      | closed |
