...
| TD-011 | Auth runtime (prod) | Для обхода Vercel timeout login временно принимает `email` из query (`?email=`).         | Email попадает в URL-логи (PII), контракт login временно неканоничный.          | Стабилизировать `req.json()` в Vercel, вернуть JSON body-only, удалить query workaround и debug routes. | High      | open   |
