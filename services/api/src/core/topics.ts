...
import { companyTable, expertTable, notificationTable, topicTable, userTable } from '../providers/db';
...
export const suggestTopics = async (
  content: ContentPort,
  input: { company: { domain: string; name: string }; experts: Array<{ id: string; name: string; role_title: string; domain: string }> },
...
export const sendWeeklyProposals = async (
  ...
  const [company] = await deps.db.select().from(companyTable).where(eq(companyTable.id, companyId)).limit(1);
  if (!company) return 0;

  const experts = await deps.db.select().from(expertTable).where(and(eq(expertTable.companyId, companyId), eq(expertTable.status, 'active')));
  ...
  const suggested = await suggestTopics(deps.content, {
    ...
    experts: experts.map((item) => ({ id: item.id, name: item.name, role_title: item.roleTitle, domain: item.domain })),
  });

  const inserted = await deps.db.insert(topicTable).values(suggested.map((item) => ({
    companyId,
  ...
  for (const manager of managers) {
    ...
    for (const topic of inserted) {
      ...
      await deps.db.insert(notificationTable).values({ companyId, recipientEmail: manager.email, notificationType: 'topic_proposal', referenceType: 'topic', referenceId: topic.id, emailToken: token, status: 'sent', sentAt: new Date() });
      emailItems.push({
        title: topic.title,
    ...
    await deps.email.sendEmail({ to: manager.email, subject: message.subject, html: message.html, textBody: message.textBody });
  }
