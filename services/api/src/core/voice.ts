...
export const calculateVoiceScore = (profile: Record<string, unknown>, text: string): number => {
  ...
  const matches = phrases.filter(
    (phrase) => typeof phrase === 'string' && text.includes(phrase),
  ).length;
...
export const generateVoiceTest = (profile: Record<string, unknown>): string => {
  const words = Array.isArray(profile.descriptive_words)
    ? profile.descriptive_words.join(', ')
    : 'clear, practical';
  return (
    `Voice test draft: this short editorial sample mirrors your style: ${words}. ` +
...
export const recordSelfRating = async (db: Database, input: RatingInput) => {
  ...
  const [draft] = await db
    ...
    .limit(1);
  if (!draft || draft.expertId !== input.expertId)
    throw new Error('expert is not assigned to this draft');
  ...
  const metadata = {
    ...(latest.diffFromPrevious as Record<string, unknown> | null),
    voice_rating: input.score,
  };
  await db
    .update(draftVersionTable)
    .set({ diffFromPrevious: metadata } as Partial<typeof draftVersionTable.$inferInsert>)
    .where(eq(draftVersionTable.id, latest.id));
  ...
  if (!belowThreshold) {
    ...
    await db
      .update(expertTable)
      .set({ status: 'active' } as Partial<typeof expertTable.$inferInsert>)
      .where(eq(expertTable.id, input.expertId));
  }
