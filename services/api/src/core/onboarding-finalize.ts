...
import { DrizzleDraftStore, expertTable, notificationTable, onboardingSequenceTable, topicTable, voiceProfileTable } from '../providers/db';
...
export const finalizeOnboardingVoiceTest = async (context: FinalizeContext, expertId: string) => {
  ...
  const responses = steps
    .filter((step) => step.responseData && typeof (step.responseData as { text?: unknown }).text === 'string')
  ...
  const [existingProfile] = await context.db
    ...
    .limit(1);
  if (existingProfile) {
    await context.db.update(voiceProfileTable).set({ profileData, updatedAt: new Date() }).where(eq(voiceProfileTable.id, existingProfile.id));
  } else {
    await context.db.insert(voiceProfileTable).values({ expertId, status: 'draft', profileData, publicTextsData: {}, voiceTestFeedback: [] });
  }

  const [topic] = await context.db
    .insert(topicTable)
    .values({ companyId: expert.companyId, expertId: expert.id, title: `Voice test: ${expert.name}`, sourceType: 'manual', status: 'approved', proposedBy: 'system' })
    .returning();
  const draftStore = new DrizzleDraftStore(context.db);
  const draft = await draftStore.create({ topicId: topic.id, expertId: expert.id, companyId: expert.companyId, status: 'drafting' });
  ...
  await context.db.update(expertTable).set({ status: 'voice_testing' }).where(eq(expertTable.id, expert.id));
  ...
  const template = buildRatingTemplate(expert.name, draft.id, emailToken, process.env.APP_URL ?? 'http://localhost:5173');
