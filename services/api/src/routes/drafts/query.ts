    ...
    drafts.map(async (draft) => {
      ...
      const [report] = version
        ? await deps.db
            ...
            .where(eq(factcheckReportTable.draftVersionId, version.id))
            .limit(1)
        : [];
      return {
        id: draft.id,
        ...
        publish_plan: {
          ...
          timezone: draft.publishTimezone,
          is_scheduled: Boolean(draft.scheduledPublishAt),
        },
...
export const getDraftDetail = (deps: RouteDeps) => async (context: Context) => {
  ...
  const [currentVersion] = draft.currentVersionId
    ? await deps.db
        ...
        .limit(1)
    : [];
  const [factcheck] = currentVersion
    ? await deps.db
        .select()
        .from(factcheckReportTable)
        .where(eq(factcheckReportTable.draftVersionId, currentVersion.id))
        .limit(1)
    : [];
  const comments = currentVersion
    ? await deps.db
        .select()
        .from(commentTable)
        .where(eq(commentTable.draftVersionId, currentVersion.id))
    : [];
  const [approvalFlow] = await deps.db
    .select()
    .from(approvalFlowTable)
    .where(eq(approvalFlowTable.draftId, draft.id))
    .limit(1);
  ...
  if (approvalFlow) {
    const steps = await deps.db
      ...
      .where(eq(approvalStepTable.approvalFlowId, approvalFlow.id))
          ...
          const [user] =
            step.approverType === 'user'
              ? await deps.db
                  .select()
                  .from(userTable)
                  .where(eq(userTable.id, step.approverId))
                  .limit(1)
              : [];
          const [approverExpert] =
            step.approverType === 'expert'
              ? await deps.db
                  .select()
                  .from(expertTable)
                  .where(eq(expertTable.id, step.approverId))
                  .limit(1)
              : [];
          return {
            ...
            approver: {
              name: user?.name ?? approverExpert?.name ?? 'Unknown',
              email: user?.email ?? approverExpert?.email ?? null,
            },
          };
        }),
      ),
