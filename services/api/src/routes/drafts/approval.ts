...
import { approvalFlowTable, draftTable, draftVersionTable, factcheckReportTable, topicTable } from '../../providers/db';
...
const parseConfig = (body: Record<string, unknown>): ApprovalConfig => {
  ...
  const deadline_hours = typeof body.deadline_hours === 'number' && body.deadline_hours > 0 ? body.deadline_hours : 48;
...
export const sendForReview = (deps: RouteDeps) => async (context: Context) => {
  ...
  const [factcheck] = await deps.db.select().from(factcheckReportTable).where(eq(factcheckReportTable.draftVersionId, draft.currentVersionId)).limit(1);
  if (!factcheck || factcheck.status !== 'completed') throw new AppError(400, 'INVALID_STATE', 'Factcheck must be completed');
  ...
  const notifications = await Promise.all(pendingSteps.map(async (step) => {
    const approver = await resolveApprover(deps, step.approverType as 'user' | 'expert', step.approverId);
  ...
  await deps.db.update(draftTable).set({ status: 'needs_review', updatedAt: new Date() }).where(eq(draftTable.id, draft.id));
  await deps.db.update(approvalFlowTable).set({ status: 'active' }).where(eq(approvalFlowTable.id, flow.id));
  ...
  return context.json({ approval_flow_id: flow.id, status: 'active', notifications_sent: notifications.length });
};
