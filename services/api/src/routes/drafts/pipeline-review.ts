...
import { buildReport, extractClaims, flagDangerousAdvice, rejectUnSourcedStats, verifyHighRiskClaims } from '../../core/factcheck';
import { AppError } from '../../core/errors';
import { DrizzleDraftStore, claimTable, draftTable, draftVersionTable, expertTable, factcheckReportTable } from '../../providers/db';
...
export const factcheckDraft = (deps: RouteDeps) => async (context: Context) => {
  ...
  if (!draft || !draft.currentVersionId) throw new AppError(400, 'INVALID_STATE', 'Draft has no current version');

  const [existingReport] = await deps.db.select().from(factcheckReportTable).where(eq(factcheckReportTable.draftVersionId, draft.currentVersionId)).limit(1);
  if (existingReport) {
    const stream = async function* () {
      yield { type: 'done', report_id: existingReport.id, overall_risk_score: existingReport.overallRiskScore, disclaimer_type: existingReport.disclaimerType };
    };
  ...
  const stream = async function* () {
    ...
    if (flagDangerousAdvice(claims).length > 0) throw new AppError(422, 'DANGEROUS_CLAIMS', 'Draft contains dangerous claims');
    ...
    for (const claim of claims) yield { type: 'claim', text: claim.text, risk_level: claim.riskLevel };

    const insertedClaims = await deps.db.insert(claimTable).values(claims.map((claim) => ({ draftVersionId: version.id, text: claim.text, claimType: claim.claimType, riskLevel: claim.riskLevel }))).returning();
    const [insertedReport] = await deps.db.insert(factcheckReportTable).values({
      draftVersionId: version.id,
      status: unsourced.length > 0 ? 'failed' : 'completed',
      results: report.results.map((item, index) => ({ ...item, claim_id: insertedClaims[index]?.id })),
      overallRiskScore: report.overallRiskScore.toFixed(2),
      disclaimerType: report.disclaimerType,
      completedAt: new Date(),
    }).returning();

    await transitionDraftStatus(deps.db, draft.id, unsourced.length > 0 ? 'revisions' : 'needs_review');
    yield { type: 'done', report_id: insertedReport.id, overall_risk_score: insertedReport.overallRiskScore, disclaimer_type: insertedReport.disclaimerType };
  };
...
export const reviseDraft = (deps: RouteDeps) => async (context: Context) => {
  ...
  const [current] = await deps.db.select().from(draftVersionTable).where(eq(draftVersionTable.id, draft.currentVersionId)).limit(1);
  ...
  const stream = async function* () {
    ...
    const chunks: string[] = [];
    for await (const chunk of await deps.content.streamText({ model: process.env.OPENROUTER_REVISE_MODEL ?? 'openai/gpt-4o-mini', prompt })) {
      chunks.push(chunk);
      yield { type: 'chunk', text: chunk };
    }
    const content = chunks.join('').trim() || current.content;
    const version = await createVersion(new DrizzleDraftStore(deps.db), draft.id, content, content.slice(0, 180), 0.82, 'revision');
