...
const parseScore = (queryScore: string | undefined, bodyScore: unknown): number => {
  ...
  if (!Number.isInteger(score) || score < 1 || score > 10)
    throw new AppError(400, 'VALIDATION_ERROR', 'score must be 1-10');
  return score;
};
...
export const handleVoiceRating = (deps: RouteDeps) => async (context: Context) => {
  ...
  const body =
    context.req.method === 'POST'
      ? await readJsonBodyStrict<{ score?: number; token?: string }>(context.req.raw)
      : ({ score: undefined, token: undefined } as { score?: number; token?: string });
  ...
  return context.json({
    ...
    revision_offered: result.revisionOffered,
  });
};
