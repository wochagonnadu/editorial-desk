...
export const createDraftComment = (deps: RouteDeps) => async (context: Context) => {
  ...
  if (typeof body.text !== 'string' || body.text.trim().length < 2) throw new AppError(400, 'VALIDATION_ERROR', 'comment text is required');
  ...
  if (!draft || !draft.currentVersionId) throw new AppError(404, 'NOT_FOUND', 'Draft version not found');

  const [comment] = await deps.db.insert(commentTable).values({
...
export const confirmClaim = (deps: RouteDeps) => async (context: Context) => {
  ...
  const [claim] = await deps.db.select().from(claimTable).where(eq(claimTable.id, claimId)).limit(1);
  if (!claim || claim.draftVersionId !== draft.currentVersionId) throw new AppError(404, 'NOT_FOUND', 'Claim not found');

  const [report] = await deps.db.select().from(factcheckReportTable).where(eq(factcheckReportTable.draftVersionId, draft.currentVersionId)).limit(1);
  ...
  const results = ((report.results as Array<Record<string, unknown>> | null) ?? []).map((item) => {
    if (item.claim_id === claimId || item.claimId === claimId) return { ...item, verdict: 'expert_confirmed' };
    return item;
  });
  await deps.db.update(factcheckReportTable).set({ results }).where(eq(factcheckReportTable.id, report.id));
