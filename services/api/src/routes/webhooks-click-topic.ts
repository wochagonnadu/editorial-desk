...
const parseTopicClick = async (context: Context, payload?: Record<string, unknown>) => {
  const body =
...
export const processTopicClick =
  (deps: RouteDeps) => async (context: Context, payload?: Record<string, unknown>) => {
    const { action, token, topicId } = await parseTopicClick(context, payload);
