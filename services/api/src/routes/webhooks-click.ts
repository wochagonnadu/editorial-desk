...
import { readJsonBodyOptional } from '../core/http/read-json-body.js';
import {
  approvalFlowTable,
  approvalStepTable,
  draftTable,
...
const parseParams = async (context: Context, payload?: Record<string, unknown>) => {
  const body =
    payload ?? (await readJsonBodyOptional<Record<string, unknown>>(context.req.raw)) ?? {};
...
export const processApprovalClick =
  (deps: RouteDeps) => async (context: Context, payload?: Record<string, unknown>) => {
    const { action, token, draftId, version, comment } = await parseParams(context, payload);
    ...
    if (notification.status === 'replied')
      throw new AppError(409, 'CONFLICT', 'token already used');
