...
import { approvalFlowTable, approvalStepTable, draftTable, draftVersionTable, notificationTable, topicTable, userTable } from '../providers/db';
import { resolveApprover, sendApprovalRequest } from './approval-notify';
import type { RouteDeps } from './deps';

const parseParams = async (context: Context) => {
  const body = (await context.req.json().catch(() => ({}))) as Record<string, unknown>;
  const action = (context.req.query('action') ?? body.action ?? '') as 'approve' | 'request_changes';
...
export const processApprovalClick = (deps: RouteDeps) => async (context: Context) => {
  ...
  const [notification] = await deps.db.select().from(notificationTable).where(and(eq(notificationTable.emailToken, token), eq(notificationTable.referenceType, 'approval_step'))).limit(1);
  if (!notification || !notification.referenceId) throw new AppError(401, 'UNAUTHORIZED', 'invalid token');
  ...
  const [step] = await deps.db.select().from(approvalStepTable).where(eq(approvalStepTable.id, notification.referenceId)).limit(1);
  ...
  if (!flow || flow.draftId !== draftId) throw new AppError(404, 'NOT_FOUND', 'approval flow not found');
  ...
  if (version && version !== currentVersion.versionNumber) throw new AppError(409, 'STALE_VERSION', 'version mismatch');

  await recordDecision(deps.db, step.id, currentVersion.id, action === 'approve' ? 'approved' : 'changes_requested', comment);

  const steps = await deps.db.select().from(approvalStepTable).where(eq(approvalStepTable.approvalFlowId, flow.id));
  const hasChanges = steps.some((item) => item.status === 'changes_requested');
  const allDone = steps.every((item) => item.status === 'approved' || item.status === 'changes_requested');

  if (flow.flowType === 'sequential' && action === 'approve') {
    const nextStep = await activateNextStep(deps.db, flow.id, flow.deadlineHours);
    if (nextStep) {
      const approver = await resolveApprover(deps, nextStep.approverType as 'user' | 'expert', nextStep.approverId);
  ...
  if (allDone) {
    await deps.db.update(approvalFlowTable).set({ status: 'completed' }).where(eq(approvalFlowTable.id, flow.id));
    await deps.db.update(draftTable).set({ status: hasChanges ? 'revisions' : 'approved', updatedAt: new Date() }).where(eq(draftTable.id, draftId));

    if (hasChanges) {
      const feedback = await consolidateFeedback(deps.db, flow.id);
      const [topic] = await deps.db.select().from(topicTable).where(eq(topicTable.id, draft.topicId)).limit(1);
      const [creator] = await deps.db.select().from(userTable).where(eq(userTable.id, flow.createdBy)).limit(1);
      if (creator) {
        const message = consolidatedFeedbackTemplate(topic?.title ?? 'Draft', feedback);
        await deps.email.sendEmail({ to: creator.email, subject: message.subject, html: message.html, textBody: message.textBody });
      }
    }
  }

  await logAudit(deps.db, { companyId: draft.companyId, actorType: step.approverType as 'user' | 'expert', actorId: step.approverId, action: `approval.${action === 'approve' ? 'granted' : 'changes_requested'}`, entityType: 'approval_step', entityId: step.id, draftVersionId: currentVersion.id, metadata: { flow_id: flow.id } });
