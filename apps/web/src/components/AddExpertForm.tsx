...
interface AddExpertFormProps {
  onSubmit(payload: { name: string; role_title: string; email: string; domain: string }): Promise<void>;
}
...
export const AddExpertForm = ({ onSubmit }: AddExpertFormProps) => {
  ...
  return (
    <form className="card" onSubmit={submit}>
      ...
      <input value={roleTitle} onChange={(event) => setRoleTitle(event.target.value)} placeholder="Role title" required />
